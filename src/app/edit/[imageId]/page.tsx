...
export default function EditImagePage() {
    ...
    if (!originalImage) {
         return (
            <div class="container mx-auto p-8 text-center">
                 <h1 class="text-2xl font-bold">Image introuvable</h1>
                 <p class="text-muted-foreground">L'image que vous essayez de modifier n'existe pas ou vous n'y avez pas accès.</p>
    ...
    return (
        <div class="bg-muted/20 min-h-screen">
            {/* -- HEADER -- */}
            <header class="sticky top-0 bg-background/80 backdrop-blur-sm border-b z-20">
                <div class="container mx-auto p-3 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        ...
                        <h1 class="text-lg font-semibold tracking-tight hidden sm:block">Édition par IA</h1>
                    </div>
                    <div class="flex items-center gap-4">
                       <Badge variant="outline" class="h-8 text-sm">
                       ...
                       <Button onClick={handleSaveAiImage} disabled={!generatedImageUrl || isSaving || isGenerating}>
                            {isSaving ? <Loader2 class="mr-2 h-4 w-4 animate-spin"/> : <Save class="mr-2 h-4 w-4" />}
                ...
                <main class="py-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-center justify-items-center">
                        <div class="w-full max-w-md flex flex-col items-center gap-2">
                            <p class="text-sm font-semibold text-muted-foreground">AVANT</p>
                            <div class="aspect-square w-full relative rounded-lg border bg-background overflow-hidden shadow-sm">
                                <Image src={originalImage.directUrl} alt="Image originale" fill sizes="(max-width: 768px) 100vw, 50vw" class="object-contain" unoptimized/>
                            </div>
                        </div>
                        <div class="w-full max-w-md flex flex-col items-center gap-2">
                            <p class="text-sm font-semibold text-muted-foreground">APRÈS</p>
                            <div class="aspect-square w-full relative rounded-lg border bg-background flex items-center justify-center shadow-sm">
                                {isGenerating && <Loader2 class="h-12 w-12 animate-spin text-primary" />}
                                {!isGenerating && generatedImageUrl && <Image src={generatedImageUrl} alt="Image générée par l'IA" fill sizes="(max-width: 768px) 100vw, 50vw" class="object-contain" unoptimized/>}
                                {!isGenerating && !generatedImageUrl && <Wand2 class="h-12 w-12 text-muted-foreground/30"/>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col items-center gap-2">
                        ...
                        >
                            {isGenerating ? <Loader2 class="mr-2 h-5 w-5 animate-spin"/> : <Sparkles class="mr-2 h-5 w-5" />}
                            {isGenerating ? 'Génération en cours...' : 'Générer avec l\'IA'}
                        </Button>
                        {!hasAiTickets && !isGenerating && (
                            <p class="text-center text-xs text-destructive font-semibold cursor-pointer hover:underline">
                    ...
                    {/* -- CONTROLS PANEL -- */}
                    <div class="rounded-lg border bg-background/95 backdrop-blur-sm shadow-sm">
                        <div class="p-4 grid grid-cols-1 lg:grid-cols-2 gap-4 items-stretch">
                            {/* Prompt Input Area */}
                            <div class="flex flex-col space-y-3">
                                <h2 class="text-base font-semibold">1. Donnez votre instruction</h2>
                                 <Textarea
                                    ...
                                    class="flex-grow min-h-[160px]"
                                />
                            </div>

                            {/* Suggestions Area */}
                            <div class="flex flex-col space-y-3 mt-4 lg:mt-0">
                                <h2 class="text-base font-semibold">2. Ou inspirez-vous</h2>
                                <div class="flex-grow w-full rounded-md border p-2 bg-muted/40 overflow-y-auto">
                                    <Accordion type="single" collapsible class="w-full">
                                        {suggestionCategories.map(category => (
                                            <AccordionItem value={category.name} key={category.name}>
                                                <AccordionTrigger class="text-sm py-2">{category.name}</AccordionTrigger>
                                                <AccordionContent>
                                                    <div class="flex flex-wrap gap-2 pt-2">
                                                        {category.prompts.map(p => (
                                                            <Button key={p.title} variant="outline" size="sm" class="text-xs h-auto py-1 px-2" onClick={() => setPrompt(p.prompt)} disabled={isGenerating || isSaving}>
                                                                {p.title}
                                                            </Button>
                                                        ))}
                                                    </div>
